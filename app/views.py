...
from django.contrib import messages #to sent messages
from .models import Customer, Product, Cart, OrderPlaced
from .forms import CustomerRegistrationForm, CustomerProfileForm
from django.views import View
from django.http import JsonResponse
from django.db.models import Q
from django.contrib.auth.decorators import login_required #for fun based views
...
#add to cart se add krwa lege n show cart islye bna rhe taki isme whi product show ho jis user ne login
# kiya ho.
@login_required
def show_cart(request): 
	...
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
		user = request.user
		cart = Cart.objects.filter(user=user)#whi cart nikalna h jis user ne login kiya hua h
		amount = 0.0
...
def plus_cart(request):
	if request.method == 'GET':
		prod_id = request.GET['prod_id'] #ye prod_id ajax se arhi h 
		c = Cart.objects.get(Q(product=prod_id) & Q(user=request.user))#wo prod_id jo mili h ajax se n usi user ka jo logged in h..usi ka obj hme milega.
		#same prod_id ke bahut sare user ho skte h but hme whi chahye jo logged in ho..then jo b obj mile use hm +1 increase kr denge.
		c.quantity+=1
		c.save()
		amount = 0.0
		shipping_amount= 70.0
		...
		for p in cart_product:
			...
			# print("After", amount)
		# print("Total", amount)
		data = {                        #qty updated hokr show hogi iski help se.
			'quantity':c.quantity,
			'amount':amount,
			'totalamount':amount+shipping_amount
		}
		return JsonResponse(data)#is response ke through waps kr denge ajax ke pas n success ka msg mil jayega.
	else:
		return HttpResponse("")
